...
const MapComponent = ({ isSidebarOpen, attackSpeed }) => {
  ...
  useEffect(() => {
    ...
    //  // Set heatmap data state
    //  setHeatmapData(attackData);
 
    //  // Create and add heatmap layer with precision tuning
    //  const heatLayer = L.heatLayer(attackData, {
    //    radius: 30,    // Reduced radius to make each spot smaller and sharper
    //    blur: 0,       // Lower blur to keep spots more focused
    //    maxZoom: 17,
    //    minOpacity: 0.4, // Make the heatmap more visible by adjusting opacity
    //    max: 20,        // Adjust max value based on the number of attacks in your dataset (20 as an example)
    //  }).addTo(map);
  
    //  // Debugging log for heatmap data
    //  console.log('Heatmap Data:', attackData);
